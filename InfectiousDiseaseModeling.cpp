...
void benchmark() {
	...
	string input_graph_filename = "patiala.edges";//(500 nodes)
	...
	size_t benchmark_max_individual_count = 503138; // 100000; // population of Patiala is 5 lakh
...
int main() {
	...
	if (do_benchmark) {
		...
		std::uint8_t total_epochs = DEFAULT_TOTAL_EPOCHS;
		std::uint8_t repeat_count = DEFAULT_REPEAT_COUNT;
		string input_graph_filename = "patiala.edges";//"minimumantwerp.edges"; // Read locations from the full Antwerp graph or from a minimal version (500 nodes)
